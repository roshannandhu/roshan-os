...
if (-not (Test-PrivateIpv4 $ControllerBindIp)) {
  throw "Could not determine a private controller LAN address. Supply ControllerBindIp explicitly; do not change firewall settings."
}
...
try {
  ...
  Write-Host "LAN_VALIDATION_READY"
  Write-Host "Open http://$ControllerBindIp`:$Port on the approved phone, then use the same temporary password."
  Write-Host "Do not change firewall settings if the phone cannot connect. Press Ctrl+C immediately after validation."
  ...
  Write-Host "LAN_VALIDATION_REMOVED"
}
